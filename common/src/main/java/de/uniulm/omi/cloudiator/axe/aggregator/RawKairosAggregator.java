 ...
 */
public abstract class RawKairosAggregator extends KairosAggregator {

    private final RawMonitor monitor;

    public RawKairosAggregator(KairosDbService kairos,
        Map<String, List<MonitorInstance>> monitorListMap, RawMonitor monitor) {
        super(kairos, monitorListMap);
        this.monitor = monitor;
    }

    public RawMonitor getMonitor() {
        return monitor;
    }

    @Override public long getMonitorId() {
        return getMonitor().getId();
    }
}
