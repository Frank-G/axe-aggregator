...
public class MeasurementWindow extends Window {
    ...
    public MeasurementWindow(long id, int measurements) {
        super(id);
    ...
    @Override public long aggregationInMilliseconds(Schedule schedule, long minimumInterval) {
        long result = (Utils.timeToMilliseconds(schedule.getTimeUnit(), schedule.getInterval())
            + minimumInterval);
        result *= this.getMeasurements();
        return result;
    }
}
